...
module ProjectEuler
  class Solution11 < BaseSolution
    ...
    def solve
      ...
      # solved:
      self.mark_as_solved
      # solution => at grid: (13, 7) - if starting at (1, 1) -> (20, 20)

      # code:
      global_max = 0
      (1..20).each do |i|
        (1..20).each do |j|
          local_max = max_by_index(n, i-1, j-1)
    ...
    def max_by_index(n, i=0,j=0)
      [ (n[i][j] * n[i+1][j] * n[i+2][j] * n[i+3][j] rescue 0),
        (n[i][j] * n[i][j+1] * n[i][j+2] * n[i][j+3] rescue 0),
        (n[i][j] * n[i+1][j+1] * n[i+2][j+2] * n[i+3][j+3] rescue 0),
        (n[i][j] * n[i+1][j-1] * n[i+2][j-2] * n[i+3][j-3] rescue 0)  ].max
    end
